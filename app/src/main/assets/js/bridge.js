...
var B=(function(){
  ...
  return {
    ...
    listTemplates:function(){try{return b?JSON.parse(b.listTemplates()):{ok:false,files:[]};}catch(e){return {ok:false,files:[]};}},
    saveTemplate:function(name,content){try{return b?JSON.parse(b.saveTemplate(name,content)):{ok:false};}catch(e){return {ok:false};}},
    useTemplate:function(name,rid,target){try{return b?JSON.parse(b.useTemplate(name,rid,target)):{ok:false};}catch(e){return {ok:false};}},
