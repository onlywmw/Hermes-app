...
public class AgentLoop implements Runnable {
    ...
    private static final String PLAN_RULES =
            ...
            + "+APK内浏览器能力(HTTPS联网fetch/相机getUserMedia(可做扫一扫)/录音getUserMedia"
            + "/体感传感器DeviceMotion/音频播放/WebGL/手柄Gamepad)"
            + "+APK内JS桥MovShell(通知notify(t,x)返回ok/no-permission/录音recordAudio(秒)+recordResult()"
            + "/震动vibrate(ms)返回布尔, false=无马达, 还有hasVibrator()自检, 按返回值如实反馈);"
            + "⚠️带条件=APK内明文HTTP被系统拦(targetSdk36, 须服务器上HTTPS或走代理), "
            + "多用户/账号体系界面、财务系统(无后端, 必须在计划里明示「演示版」及边界);"
            + "❌做不了=打包APK内定位/agent联网抓取/真后端/支付接口/应用推送"
