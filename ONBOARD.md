...
Android 手机上，拉多个 AI 模型进房间一起干活。两个 tab：**会话**（房间+聊天+文件）和 **运行**（设备状态+AI 模型）。
...
7. `docs/ARCHITECTURE.md` 末尾 M1–M4 节 — 内嵌 Linux（shell.exec）、内嵌 Hermes 委派、交互式终端、全栈交付（写后端+ssh 部署）的架构与真机约束
...
- **desk 是普通房间（2026-07-24 起）**：id='desk' 仅首装播种，可删可改不复活。
